  ...
  <div>
    <div v-for="item in product" v-bind:key="item.name">
      <v-card
        ...
        elevation="2"
      >
        <v-img v-bind:src="item.photo"></v-img>
        ...
        <v-card-actions :class="$style.espaciado">
          ...
          <v-btn text color="teal accent-4" @click="revealText(item.id)">
            Leer más...
          </v-btn>
        </v-card-actions>
...
export default {
  ...
  methods: {
    ...
    showCategory(atributtes) {
      ...
      atributtes.forEach((obj) => {
        if (obj.name.toUpperCase() == newCategory.toUpperCase()) {
          showProduct = true;
          return true;
        }
      });
  ...
  computed: {},
  created() {
    axios.get("http://sva.talana.com:8000/api/product/").then((resp) => {
